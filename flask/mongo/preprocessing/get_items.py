...
for key, values in food_categories.items():
    ...
    endpoint = f"https://api.edamam.com/api/food-database/v2/parser?app_id={id}&app_key={api_key}&ingr={values}&nutrition-type=cooking"
    if calls >= 8:
        time.sleep(60)
        calls = 0
    ...
    if response.status_code == OK:
        if len(response.json()) > 0:
            ...
            items_preprocessing(json_file_path, values)
        calls += 1
    else:
        print(response)
